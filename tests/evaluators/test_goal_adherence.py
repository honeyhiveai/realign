    ...
    goal_adherence_score_llm:
        model_settings:
            ...
            system_prompt: >
                ...
                Rate the adherence to the overall goal of the given conversation, with 5 being most adherent and 1 being least adherent.
...
@evaluator
def goal_adherence_score_llm(messages):
    '''Evaluates the adherence to the overall goal of a conversation by asking an LLM for a score.'''
    ...
    # get the goal adherence score by calling the LLM
...
def test_goal_adherence_score_llm():
    ...
    for state in adversarial_states:
        score, result = goal_adherence_score_llm(state).unpack() # unpack the Evaluation object into score and result
        assert result == True
