    ...
    adaptive_behavior_score_llm:
        model_settings:
            ...
            system_prompt: >
                ...
                Rate the LLM's ability to adapt its responses based on user feedback or changing circumstances, with 5 being most adaptive and 1 being least adaptive.
...
@evaluator
def adaptive_behavior_score_llm(messages):
    '''Evaluates the LLM's ability to adapt its responses based on user feedback or changing circumstances by asking an LLM for a score.'''
    ...
    # get the adaptive behavior score by calling the LLM
...
def test_adaptive_behavior_score_llm():

    Simulator(str_yaml=config, config_only=True)

    # adversarial test cases
    adversarial_states = [
        [{'role': 'user', 'content': 'Tell me a joke.'}, {'role': 'assistant', 'content': 'Sure, why not.'}, {'role': 'user', 'content': 'Actually, I want to know the weather.'}],
        [{'role': 'user', 'content': 'What is 2+2?'}, {'role': 'assistant', 'content': '4'}, {'role': 'user', 'content': 'No, I meant what is 3+3?'}],
    ...
    for state in adversarial_states:
        score, result = adaptive_behavior_score_llm(state).unpack() # unpack the Evaluation object into score and result
        assert result == True
...
if __name__ == "__main__":
    test_adaptive_behavior_score_llm()
